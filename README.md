...
This repo currently contains only Dart source (`lib/`), config, and assets. The
native platform folders are generated by the Flutter tool and are **not** checked
in. From the project root:

```bash
flutter create .          # generates android/, ios/, etc. — does NOT touch lib/
flutter pub get
```

Then grant the required capabilities:
...
- **iOS** (`ios/Runner/Info.plist`):
  ...
  <string>Position GPS pour la navigation VFR.</string>
  ```
